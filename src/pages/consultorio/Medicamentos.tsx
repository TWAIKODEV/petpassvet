...
import { Plus, Search, Download, Pill, RefreshCw, Edit, Trash, Tag, Eye, X, DollarSign, Database, AlertTriangle } from 'lucide-react';
...
import NewMedicineForm from '../../components/form/NewMedicineForm';
...
const Medicamentos: React.FC = () => {
  ...
  const types = Array.from(new Set(medications.map((med: Medication) => med.type)));
  const species = Array.from(new Set(medications.flatMap((med: Medication) => med.species)));
  ...
  const filteredMedications = medications.filter((medication: Medication) => {
    const matchesSearch = 
      ...
      medication.conditions.some((c: string) => c.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const lowStockMedications = medications.filter((med: Medication) => med.stock <= med.minStock).length;
  const totalValue = medications.reduce((sum: number, med: Medication) => sum + (med.stock * med.basePrice), 0);
  ...
  return (
    <div className="space-y-6">
            ...
            <Card key={medication._id}>
              <div className="p-6">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  {medication.species.map((species: string) => (
                    <span key={`${medication._id}-${species}`} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                      {species}
                    </span>
                  ))}
      ...
      {showNewMedicationForm && (
        <NewMedicineForm
          isOpen={showNewMedicationForm}
          onClose={() => setShowNewMedicationForm(false)}
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex flex-wrap gap-1">
                        {selectedMedication.species.map((species: string) => (
                          <span key={`${selectedMedication._id}-${species}`} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                            {species}
                          </span>
                        ))}
                      </div>
                    ...
                    {selectedMedication.administrationRoutes && selectedMedication.administrationRoutes.length > 0 && (
                      <div>
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          {selectedMedication.administrationRoutes?.map((route: string) => (
                            <span key={`${selectedMedication._id}-route-${route}`} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                              {route}
                            </span>
                          ))}
                        </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex flex-wrap gap-1">
                        {selectedMedication.conditions.map((condition: string) => (
                          <span key={`${selectedMedication._id}-condition-${condition}`} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                            {condition}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <ul className="mt-1 text-sm text-gray-900 list-disc list-inside">
                        {selectedMedication.contraindications.map((item: string) => (
                          <li key={`${selectedMedication._id}-contraindication-${item}`}>{item}</li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      ...
                      <ul className="mt-1 text-sm text-gray-900 list-disc list-inside">
                        {selectedMedication.sideEffects.map((item: string) => (
                          <li key={`${selectedMedication._id}-sideEffect-${item}`}>{item}</li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      ...
                      <ul className="mt-1 text-sm text-gray-900 list-disc list-inside">
                        {selectedMedication.interactions.map((item: string) => (
                          <li key={`${selectedMedication._id}-interaction-${item}`}>{item}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
